...
export async function GET() {
  try {
    ...
    const recentActivityRate24h =
      totalUsers > 0 ? (usersWithRecentScrobbles24h / totalUsers) * 100 : 0;
    ...
    return NextResponse.json({
      ...
      avgScrobblesPerActiveUser:
        activeUsers > 0 ? Math.round(totalScrobbles / activeUsers) : 0,
      topArtists: topArtists.map((a) => ({
        artist: a.artist,
        count: a._count.artist,
      })),
      failureStats: failureStats.reduce(
        (acc, f) => {
